...
@router.post("")
async def create_image_job(user=Depends(require_user)) -> dict:
    ...
    if not limiter.allow(user["sub"], LeakyBucketConfig(capacity=20, leak_rate_per_sec=1.0)):
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Rate limited")
    cost = 10
    new_balance = await credits.debit(user["sub"], cost, "generate_image")
    job = await jobs.create_job(user["sub"], "image", {"prompt": ""}, cost, provider="replicate")
    enqueue_generate("image", job["id"])  # fire-and-forget
